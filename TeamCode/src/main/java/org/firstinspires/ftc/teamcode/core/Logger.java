...
public class Logger {
    ...
    public void updateMechanismLevel(boolean Left_Bumper) {
        ...
        if (state == LogLevels.DRIVER_DATA) {
            telemetry.addLine("Current Logger State: DRIVER_DATA");
        } else if (state == LogLevels.DEBUG) {
            telemetry.addLine("Current Logger State: DEBUG");
        } else if (state == LogLevels.PRODUCTION) {
            telemetry.addLine("Current Logger State: PRODUCTION");
        }
        if (mechanismState == MechanismLog.INTAKE_TRANSFER) {
            telemetry.addLine("MECHANISM STATE: INTAKE_TRANSFER");
        } else if (mechanismState == MechanismLog.LAUNCHER) {
            telemetry.addLine("MECHANISM STATE: LAUNCHER");
        }
    }
    ...
    public boolean LAUNCHER() {
        return mechanismState == MechanismLog.LAUNCHER;
    }
}
